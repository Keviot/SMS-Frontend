...
const questions = [
    {
        ...
        description: "Feel free to let me know if you need more examples or if there's anything specific you'd like to include in your dummy content! Feel free to let me know if you need more examples or if there's anything specific you'd like to include in your dummy content.",
        votes: 0,
        answersCount: 0,
        views: 20,
    },
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-[#F4F4F4] px-6 py-4 bg-white sticky top-0 z-10">
                    <div className="flex items-center gap-3">
                        {view !== "list" && (
                            <button onClick={() => setView("list")} className="mr-2 p-1 rounded-full hover:bg-gray-100 transition-colors">
                                <ChevronLeft size={20} />
                            </button>
                        )}
                                ...
                                >
                                    <div className="flex flex-col items-center gap-3 min-w-[80px]">
                                        <div className="text-center">
                                            ...
                                            <p className={cn("text-sm font-bold", q.color || "text-[#202224]")}>{q.votes}</p>
                                        </div>
                                        <div className="text-center">
                                            <p className="text-[10px] font-bold text-[#A7A7A7] uppercase tracking-wider mb-1">answers</p>
                                            <p className={cn("text-sm font-bold", q.answersCount > 0 ? "text-[#5678E9]" : "text-[#202224]")}>{q.answersCount}</p>
                                        </div>
                                    </div>

                                    <div className="flex-1">
                                        <div className="flex items-center justify-between mb-2">
                                            <h4 className="text-sm font-bold text-[#202224] group-hover:text-[#5678E9] transition-colors">{q.title}</h4>
                    ...
                    {view === "ask" && (
                        <div className="max-w-4xl mx-auto space-y-6">
                            <div className="rounded-2xl border-2 border-[#D3D3D3] border-dashed p-8 bg-[#F1F4FF]/50">
                                <h4 className="text-lg font-bold text-[#202224] mb-4">Writing a good question</h4>
                                <p className="text-sm text-[#202224] mb-4">
                                    You're ready to <span className="text-[#5678E9] cursor-pointer">ask a programming-related question</span> and this form will help guide you through the process.<br />
                                    Looking to ask a non-programming question? See <span className="text-[#5678E9] cursor-pointer">the topics here</span> to find a relevant site.
                                </p>
                            ...
                            <div className="rounded-2xl bg-white p-8 border border-[#F4F4F4] shadow-sm space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <p className="text-[11px] text-[#A7A7A7]">Be specific and imagine you're asking a question to another person.</p>
                                    <input
                                        type="text"
                                ...
                                <Button
                                    onClick={() => { toast.success("Question posted!"); setView("list"); }}
                        ...
                        <div className="max-w-5xl mx-auto space-y-8 pb-12">
                            ...
                            <div className="flex gap-6">
                                <div className="flex flex-col items-center gap-3 py-2">
                                    <button onClick={() => setVoteCount(v => v + 1)} className="p-2 rounded-full border border-[#D3D3D3] hover:bg-[#F1F4FF] transition-colors"><ArrowUp size={18} /></button>
                                    <span className="text-lg font-bold text-[#FF6B35]">{voteCount}</span>
                                    <button onClick={() => setVoteCount(v => v - 1)} className="p-2 rounded-full border border-[#D3D3D3] hover:bg-[#F1F4FF] transition-colors"><ArrowDown size={18} /></button>
                                </div>

                                <div className="flex-1 space-y-6">
                                    <div className="flex items-center justify-between">
                                        <h2 className="text-xl font-bold text-[#202224]">{selectedQuestion.title}</h2>
                                        <div className="flex items-center gap-1.5 text-[#A7A7A7]">
                                            <Eye size={14} />
                                    ...
                                    <div className="space-y-4">
                                        <h4 className="text-sm font-bold text-[#5678E9]">Answers</h4>
                                        <div className="space-y-6 pl-4 border-l-2 border-[#F4F4F4]">
                                            {[1, 2].map((i) => (
                                                <div key={i} className="space-y-2">
                                                    <p className="text-[13px] leading-relaxed text-[#A7A7A7]">
                                                        {i}. Feel free to let me know if you need more examples or if there's anything specific you'd like to include in your dummy content! Feel free to let me know if you need more examples or if there's anything specific you'd like to include in your dummy content!
                                                    </p>
                                                </div>
                                            ))}
                                        </div>
                            ...
                            {/* Answer Box */}
                            <div className="space-y-4">
                                <label className="text-sm font-bold text-[#202224]">Your Answer</label>
                                <textarea
